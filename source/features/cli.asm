...
type_time:
	...
	xor ax, ax			; zero extend 5 bits
	mov al, dh
	shr ax, 3			; 11 (start in word) - 8 (byte)
	cmp al, 10			; always 'hh'
	jae .hh
	push ax
	mov ax, 0x0e30			; '0'
  ...
  .hh:
	...
	mov ax, dx
	shr ax, 4
	and ax, 0x3f			; 7 bits for minutes
	cmp al, 10
	jae .mm
	push ax
	mov ax, 0x0e30
  ...
  .mm:
	...
	mov ax, dx
	and ax, 0xf			; 4 bits for seconds/2
	shl ax, 1
	cmp al, 10
	jae .ss
	push ax
...
; =====================================================================
	...
	prompt			db 'WinOS> ', 0
        ...
        space_text             db ' ', 13, 10, 0
        line_text              db '-------------------------------------------------------------------------------', 13, 10, 0
	help_text		db 'DIR           List Directories',13,10, 'LS            List Files',13,10, 'COPY          Copy a file',13,10, 'REN           Rename a file',13,10, 'DEL           Delete a file',13,10,'CAT           Display the content of a file',13,10,'SIZE          Display a size of a file',13,10,'CLS           Clear the terminal',13,10,'CLEAR         Clear the terminal',13,10,'HELP          Shows the help page',13,10,'TIME          Display the fime',13,10,'DATE          Shows the date (that is set in the system)',13,10,'VER           Shows the version of the OS',13,10,'EXIT          Exit the terminal',13,10,'ABOUT         About the operating system', 13, 10, 0
        about_text		db '[ABOUT] WinOS is a basic operating system, made my WinsDominoes base on MikeOS', 13, 10, 0
	invalid_msg		db '[WinOS] Bad command, do HELP for more commands', 13, 10, 0
	...
	version_msg		db '-----------', 13, 10, '|  WinOS  |', 13, 10, '-----------', 13, 10, 'WinOS Console ', MIKEOS_VER, 13, 10,' ', 13, 10, 0
